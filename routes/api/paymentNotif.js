...
function parseAmountFromText(text) {
  ...
  // Pola 1: Rp 50.125 atau Rp. 50.125 atau Rp50.125
  ...
  // Pola 3: Angka ribuan dengan titik (contoh 50.125 atau 100.025)
...
router.post(['/v1/payment-notif', '/payment-notif'], async (req, res) => {
  ...
  logger.info(`[Payment Gateway Webhook] Nominal terdeteksi: Rp ${parsedAmount.toLocaleString('id-ID')}. Mencari tiket deposit pending...`);

  // Cari tiket deposit pending dengan nominal unik tepat yang sama
  try {
    ...
    if (!deposit) {
      logger.info(`[Payment Gateway Webhook] Tidak ada tiket deposit pending dengan nominal Rp ${parsedAmount.toLocaleString('id-ID')}`);
      return res.json({
        success: true,
        matched: false,
        parsed_amount: parsedAmount,
        message: `Nominal Rp ${parsedAmount.toLocaleString('id-ID')} terdeteksi, namun tidak ada tiket deposit pending yang sesuai.`
      });
    }

    // Ada tiket cocok! Otomatis setujui (Approve) deposit agen
    logger.info(`[Payment Gateway Webhook] MATCH! Menyetujui deposit #${deposit.id} (${deposit.deposit_code}) untuk Agen #${deposit.agent_id}`);
    ...
    logger.error(`[Payment Gateway Webhook] Error memproses deposit: ${err.message}`);
